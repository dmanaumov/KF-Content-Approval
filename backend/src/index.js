...
app.get('/api/team/tasks', teamAuth.requireTeamAuth, async (req, res) => {
  ...
  try {
    ...
    // archivedProjectIds: hides archived projects from the project FILTER
    // above the list/calendar (frontend/team.js renderProjectFilterOptions) —
    // existing cards under an archived project still show up completely
    // normally in the list itself, only the filter dropdown hides that
...
app.get('/api/team/projects', teamAuth.requireTeamAuth, async (req, res) => {
  ...
  try {
    ...
    // Архивные проекты не показываем — не должны мешать при выборе проекта
